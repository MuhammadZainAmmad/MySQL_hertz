...
pd.options.display.float_format = '{:,.4f}'.format # for dealing with large float values in resultset dfs
list_responseSets = []
        ...
        for i in range(1, len(res.columns), 4):
            ...
            if len(df) > 35:
                for firstRow in range(0, len(df)-29, 30):
                    ...
                    markdown_table = splitRes.to_markdown(index=False, tablefmt="grid", floatfmt=".4f")
                    response = client.chat_postMessage(channel=channel_id, text= f"```{markdown_table}```")
            else:
                markdown_table = df.to_markdown(index=False, tablefmt="grid", floatfmt=".4f")
                response = client.chat_postMessage(channel=channel_id, text= f"```{markdown_table}```")
    else:
        if len(res) > 35:
            ...
            markdown_table = res.to_markdown(index=False, tablefmt="grid", floatfmt=".4f")
            response = client.chat_postMessage(channel=channel_id, text= f"```{markdown_table}```")
...
gs = gc.open_by_key('1FwaXhwL4jF3a3AstjOsrzCyEmaCA8G2POEuSWSucCcg') # open a google sheet

# ********************************* Filling Google Sheets with result sets *****************************
for resIndx in range(len(list_responseSets)):  
    df = list_responseSets[resIndx] 
    sheetTitle = f'Sheet{resIndx+1}'
    # if (gs.worksheet(sheetTitle)):
    #     gs.del_worksheet(sheetTitle)
    worksheet = gs.worksheet(sheetTitle)
    worksheet.clear()
